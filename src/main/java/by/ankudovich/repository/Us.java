...
public class Us implements UserRepository {
    private final String ADD_USER = "insert into user (id, \"name\",surname, login, password, role) " + "values (?,?,?,?,?,?)";
    ...
    public User add(User user) {
        try (Connection connection = JDBC.getConnection();
            ...
            if (userCount == 0) {
                preparedStatement = connection.prepareStatement("INSERT INTO user (id, \"name\", surname, login, password, role) VALUES (?, ?, ?, ?, ?, ?)");
